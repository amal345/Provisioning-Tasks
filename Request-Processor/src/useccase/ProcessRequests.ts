        ...
        for (const key in payload) {
            if (Object.prototype.hasOwnProperty.call(payload, key)) {
                ...
                if (mapping.get(request.requestCategory) === simdetails.state) {
                    const requestResult: any = await RequestProcess.saveRequest(request.simNumber, request.requestCategory, request.requestedBy)
                    new Produce().kafkaProducer(requestResult)
                }
                else {
                    throw new Error(`Your sim is not in ${mapping.get(request.requestCategory)}`)
                }
            }
        }  
    ...
    static async updateRequest(payload: any, requestId?: any) {
        try {
            ...
            if (requestId) {
                ...
                const result: string = await RequestProcess.updateRequest(requestId, payload.status)
                return result;
            }
            else {
                ...
                console.log(payload);
                status = "Completed"
                const result: string = await RequestProcess.updateRequest(payload.requestId, status)
                return result;
            }
        }
        catch (error) {
